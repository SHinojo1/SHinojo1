...
const validateCred = (array) => {
  // digit to hold running sum including check digit
  let sum = array[array.length-1];
  // reverse loop through array to start checking numbers from the second to the right value
  for (let i = array.length - 2; i >= 0; i--){
    // determine if array length is even or odd for skipping every other number
...
const findInvalidCard = (nestedArray) => {
  // new nested array to host invalid cards
  let invalidCards = [];
  // loop through nested array and call validateCred
...
const idInvalidCardCompanies = (nestedArray) => {
  // new nested array to host company names
  let invalidCardCompanies = [];
  // loop through nested array of invalid cards
  for (let array of nestedArray){
